...
# ----------------------------------------------------
# Planar degenerate boundary quantities
# ----------------------------------------------------
nu = FacetNormal(mesh)
I = Identity(2)
...
# Q_perp for trial function
Qperp_trial = Pi * Q_trial * Pi

# Q_perp for old function
Qperp_old = Pi * Q_old * Pi

# Qtilde evaluated explicitly at old iterate
...
# RHS boundary term:
# - (w1*s0/2) nu tensor nu
boundary_rhs = -(w1*s0/2) * outer(nu, nu)


# ----------------------------------------------------
# Gradient descent weak form with planar degenerate anchoring
#
# Boundary condition:
#
# l1 dQ/dnu
...
#
# We treat:
#   w1(Q - Qperp) implicitly
#   cubic surface term explicitly at Q^k
# ----------------------------------------------------
A = (
...
VTKFile("pointwise_error_planar_gradient_descent.pvd").write(pointwise_error)
...
plt.xlabel("x")
plt.ylabel("y")
plt.title("Director line field: planar degenerate anchoring")
plt.tight_layout()
plt.savefig("director_linefield_planar_gradient_descent.png", dpi=300)
plt.close()

print("Saved director_linefield_planar_gradient_descent.png")


# ----------------------------------------------------
# Save PVD files
# ----------------------------------------------------
VTKFile("q_solution_planar_gradient_descent.pvd").write(q)
VTKFile("Q_components_planar_gradient_descent.pvd").write(Q00, Q01, Q10, Q11)
VTKFile("director_planar_gradient_descent.pvd").write(director)
VTKFile("scalar_order_planar_gradient_descent.pvd").write(scalar_order)

print("Saved planar degenerate gradient descent files.")